...
const StudentForm = ({ type, data }: { data?: teacherInput; type: "create" | "update" }) => {
  const {
    register,
    handleSubmit,
    control,
  ...
  const handleSubmitStudent: SubmitHandler<teacherInput> = async (formData) => {
  ...
  return (
    <form onSubmit={handleSubmit(handleSubmitStudent)} className="flex flex-col gap-5">
      {/* Title */}
